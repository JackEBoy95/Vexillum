...
<head>
  ...
  <style>
    ...
    .hero { padding: 40px 0 48px; border-bottom: 1px solid var(--border); margin-bottom: 48px; }
    ...
    .design-btn { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: var(--accent); color: var(--accent-fg); font-weight: 700; font-size: 14px; border-radius: 8px; text-decoration: none; transition: background 0.12s; margin-top: 6px; }
    .design-btn:hover { background: var(--accent-h); }
    ...
    .flag-card { background: var(--panel-bg); border: 1px solid var(--border); border-radius: 10px; overflow: hidden; box-shadow: var(--shadow-sm); text-decoration: none; color: var(--text); transition: box-shadow 0.15s, transform 0.15s; padding: 14px 16px; display: flex; align-items: center; gap: 10px; }
    .flag-card:hover { box-shadow: var(--shadow-md); transform: translateY(-2px); }
    .flag-card-name { font-size: 13px; font-weight: 600; }
    @media (max-width: 650px) { #site-nav { padding: 0 16px; gap: 12px; } .page-wrap { padding: 0 16px 60px; } .cta-band { padding: 28px 18px; } }
  </style>
</head>
<body>
<nav id="site-nav">
...
<main class="page-wrap">
  ...
  <section class="hero">
    <div class="hero-meta">
      ...
      <a href="${remixUrl}" class="design-btn" title="Design your own flag in Quick Flags">
  ...
  <div class="cta-band">
    ...
    <p>${flag.ctaBody}</p>
    <a href="../index.html" class="btn-white">Open Flag Designer →</a>
  </div>
</main>
</body>
</html>`;
  ...
  const byContinent = continents.map(cont => {
    ...
    const cards = group.map(f => `
        <a href="${f.slug}.html" class="flag-card">
          <span class="flag-card-name">${f.name}</span>
        </a>`).join('');
    return `
    <section class="section">
